          ...
          {posts.map((post) => (
            <div key={post.id} className="bg-white mb-6 overflow-hidden shadow-sm sm:rounded-lg">
              <div className="text-gray-900">
                <div>
                  <Link
                    className="flex items-center space-x-4 pb-6 md:pb-0 pt-6 px-6"
                    href={route('profile.show', post.user)}
                  >
                    <img
                      className="rounded-full w-16 h-16"
                      src={post.user.image?.url ?? '/default.png'}
                      alt={`${post.user.name}'s Profile Picture`}
                    />
                    <div className="flex flex-col">
                      <p className="font-bold">{post.user.name}</p>
                      <small className="font-slate-600">
                  ...
                  {post.description && post.description !== '' && (
                    <p className="pt-6 md:pt-0 px-6">
                      <Link className="font-black" href={route('profile.show', post.user)}>
                        @{post.user.username}
                      </Link>{' '}
                      {post.description}
                    </p>
      ...
      <CustomModal show={openLikeModal} onClose={() => setOpenLikeModal(false)} title="Likes">
        <ul className="flex flex-col space-y-3 p-3">
          {JSON.stringify(currentPost) !== '{}' &&
            [...currentPost.likes].map((like) => <CustomModalUser key={like.user.id} user={like.user} />)}
        </ul>
      </CustomModal>
    </AuthenticatedLayout>
  )
